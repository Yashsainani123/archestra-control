...
const mockAlerts: Security_Alert[] = [
...
const mockCostAnalytics: Cost_Analytics = {
  totalSpend: 18420,
  totalSaved: 442080,
  savingsPercent: 96,
  burnRate: 283.7,
...
const mockAuditLog: Audit_Log_Event[] = [
...
export async function executeAction(actionType: string, agentId: string): Promise<{ success: boolean; message: string }> {
  await delay(800);
  if (actionType === "kill") {
    return { success: true, message: `Kill switch executed for ${agentId}. Agent terminated.` };
  }
